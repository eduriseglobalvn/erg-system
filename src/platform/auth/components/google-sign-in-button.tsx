...
} from "@/platform/auth/api/google-identity";
...
export const GoogleSignInButton = memo(function GoogleSignInButton({
    ...
    async function initGoogle() {
      ...
      if (containerRef.current) {
        google.renderButton(containerRef.current, {
          theme: "outline",
          size: "large",
          width: isIconVariant ? 48 : containerRef.current.offsetWidth,
          shape: "pill",
        });
        setIsLoaded(true);
      }
  ...
  }, [isIconVariant, onCredential, onError]);

  if (isIconVariant) {
    return (
      <div className="relative h-12 w-12 overflow-hidden rounded-2xl transition-all duration-300">
        <div
          ref={containerRef}
          className={cn(
        ...
        <div
          aria-label={label}
          className={cn(
            "flex h-12 w-12 items-center justify-center rounded-2xl bg-[#f8fbff] shadow-[0_10px_22px_rgba(15,23,42,0.08)] transition-all duration-300 active:scale-[0.96]",
