...
export type HocLieuTreeNodeKind = "category" | "topic" | "section" | "bookSeries" | "folder";

export type HocLieuTreeNode = {
  id: string;
  label: string;
  kind: HocLieuTreeNodeKind;
  description?: string;
  optionId?: string;
  status?: string;
  metadata?: Record<string, string>;
  children: HocLieuTreeNode[];
};

export type HocLieuTreeSubject = {
  id: string;
  label: string;
  description?: string;
  status?: string;
  metadata?: Record<string, string>;
  tree: HocLieuTreeNode[];
...
function buildChildNodes(
  parentKind: HocLieuTreeNodeKind,
  parentId: string,
  allCategories: HocLieuTaxonomyOption[],
  model: HocLieuTaxonomyResponse,
  visited: Set<string>,
): HocLieuTreeNode[] {
  const nodes: HocLieuTreeNode[] = [];
  const pushed = new Set<string>();
  const push = (key: string, node: HocLieuTreeNode) => {
...
export function buildHocLieuSubjectTree(subject: HocLieuTaxonomyOption, model: HocLieuTaxonomyResponse): HocLieuTreeNode[] {
...
export function countHocLieuTreeNodes(nodes: HocLieuTreeNode[]): number {
  return nodes.reduce((total, node) => total + 1 + countHocLieuTreeNodes(node.children), 0);
}

export function findHocLieuTreeNode(nodes: HocLieuTreeNode[], nodeId: string): HocLieuTreeNode | undefined {
  for (const node of nodes) {
    if (node.id === nodeId) return node;
    const child = findHocLieuTreeNode(node.children, nodeId);
    if (child) return child;
  }
  return undefined;
}

export function findHocLieuTreePath(nodes: HocLieuTreeNode[], nodeId: string, trail: HocLieuTreeNode[] = []): HocLieuTreeNode[] {
  for (const node of nodes) {
    const nextTrail = [...trail, node];
    if (node.id === nodeId) return nextTrail;
    const childPath = findHocLieuTreePath(node.children, nodeId, nextTrail);
    if (childPath.length) return childPath;
  }
  return [];
}

export function flattenHocLieuTreeNodes(nodes: HocLieuTreeNode[]): HocLieuTreeNode[] {
  return nodes.flatMap((node) => [node, ...flattenHocLieuTreeNodes(node.children)]);
}

export function collectHocLieuLeafNodeIds(node: HocLieuTreeNode): string[] {
  if (node.kind === "section" && node.children.length === 0) {
    return [node.id];
  }
  return node.children.flatMap((child) => collectHocLieuLeafNodeIds(child));
}

export function buildHocLieuSubjects(model: HocLieuTaxonomyResponse, resources: HocLieuResourceCard[]): HocLieuTreeSubject[] {
  return sortOptions(model.subjects).map((subject) => {
    const tree = buildHocLieuSubjectTree(subject, model);
    return {
      id: subject.id,
      label: subject.label,
      ...
      tree,
      categoryCount: countHocLieuTreeNodes(tree),
