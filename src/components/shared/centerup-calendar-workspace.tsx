...
export type CenterUpCalendarTone = "blue" | "green" | "purple" | "orange" | "red" | "teal" | "pink" | "indigo";

export type CenterUpCalendarEvent = {
  attendees: string;
  className?: string;
  date: string;
  duration: string;
  ...
  time: string;
  title: string;
  tone: CenterUpCalendarTone;
};

export type CenterUpCalendarLaneSummary = {
  color: string;
  count: number;
  id?: string;
  label: string;
...
export type CenterUpCalendarResolvedTone = {
  background: string;
  border: string;
  text: string;
};

export type CenterUpScheduleSchool = {
  id: string;
  name: string;
};

export type CenterUpScheduleClass = {
  gradeLabel?: string;
  id: string;
  name: string;
  schoolId: string;
...
export type CenterUpScheduleTeacher = {
  id: string;
  name: string;
};

export type CenterUpScheduleOption = {
  id: string;
  name: string;
  schoolId?: string;
  subjectId?: string;
...
export type CenterUpScheduleCatalog = {
  assistantTeachers?: CenterUpScheduleTeacher[];
  classes: CenterUpScheduleClass[];
  levels?: Array<string | CenterUpScheduleOption>;
  rooms?: Array<string | CenterUpScheduleOption>;
  schools: CenterUpScheduleSchool[];
  subjects?: Array<string | CenterUpScheduleOption>;
  teachers: CenterUpScheduleTeacher[];
};

export type CenterUpScheduleDraftRow = {
...
export type CenterUpScheduleDraft = {
  applyFrom: string;
  note?: string;
  repeatWeeks: number;
  resolveMode?: "REJECT" | "DELETE_CONFLICTING";
  rows: CenterUpScheduleDraftRow[];
  schoolId: string;
};

export type CenterUpSchedulePreviewConflict = {
...
export type CenterUpSchedulePreviewRowResult = {
  clientRowId: string;
...
export type CenterUpSchedulePreviewResult = {
  conflicts: CenterUpSchedulePreviewConflict[];
  previewEventCount: number;
  rowResults: CenterUpSchedulePreviewRowResult[];
  teacherScheduleCount: number;
  totalPeriodCount: number;
  valid: boolean;
};

export type CenterUpCalendarVisibleRange = {
  activeLane: string;
  from: string;
  mode: CalendarMode;
...
export type CenterUpCalendarPermissions = {
  canCreate?: boolean;
  canDelete?: boolean;
  canPublish?: boolean;
  canUpdate?: boolean;
...
type CenterUpCalendarWorkspaceProps = {
  allLaneLabel?: string;
  ...
  initialDate?: string;
  lanes: string[];
  laneSummaries?: CenterUpCalendarLaneSummary[];
  monthCountLabel?: string;
  monthCountOverride?: number;
  errorMessage?: string;
  isLoading?: boolean;
  initialMode?: CalendarMode;
  onCreateSchedule?: (draft: CenterUpScheduleDraft) => Promise<void> | void;
  onPreviewSchedule?: (draft: CenterUpScheduleDraft) => Promise<CenterUpSchedulePreviewResult> | CenterUpSchedulePreviewResult;
  onRetry?: () => void;
  onVisibleRangeChange?: (range: CenterUpCalendarVisibleRange) => void;
  permissions?: CenterUpCalendarPermissions;
  resolveEventScopeLabel?: (event: CenterUpCalendarEvent, activeLane: string) => string;
  resolveEventTone?: (event: CenterUpCalendarEvent, activeLane: string) => CenterUpCalendarResolvedTone;
  resolveEventSummaryLabel?: (event: CenterUpCalendarEvent, activeLane: string) => string;
  resolveCalendarListTitle?: (activeLane: string) => string;
  resolveLaneScopeOptions?: (activeLane: string, events: CenterUpCalendarEvent[]) => CenterUpCalendarLaneSummary[];
  resolveLaneSummaries?: (activeLane: string, events: CenterUpCalendarEvent[], selectedScopeLabel: string) => CenterUpCalendarLaneSummary[];
  resolveScopeSelectLabel?: (activeLane: string) => string;
  scheduleCatalog?: CenterUpScheduleCatalog;
...
const toneMap: Record<CenterUpCalendarTone, { background: string; border: string; text: string }> = {
...
export function CenterUpCalendarWorkspace({
  allLaneLabel = "Tất cả",
...
}: CenterUpCalendarWorkspaceProps) {
  ...
  const [detailEvent, setDetailEvent] = useState<CenterUpCalendarEvent | null>(null);
  ...
  const getEventTone = useMemo(
    () =>
      resolveEventTone
        ? (event: CenterUpCalendarEvent) => resolveEventTone(event, calendarLane)
        : (event: CenterUpCalendarEvent) => toneMap[event.tone],
  ...
  const eventsByDate = useMemo(() => {
    const grouped = new Map<string, CenterUpCalendarEvent[]>();
  ...
  const visibleRange = useMemo<CenterUpCalendarVisibleRange>(() => {
    if (mode === "day") {
      return {
  ...
  function openEventDetail(event: CenterUpCalendarEvent, anchorEl: HTMLElement) {
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="vi">
      <Box sx={calendarWorkspaceSx}>
        ...
        <EventDetailPopover anchorEl={detailAnchorEl} event={detailEvent} onClose={closeEventDetail} />
        {isLoading ? <CalendarLoadingOverlay /> : null}
        <TeachingScheduleDialog catalog={scheduleCatalog ?? emptyScheduleCatalog} onClose={() => setCreateDialogOpen(false)} onPreview={onPreviewSchedule} onSubmit={onCreateSchedule} open={createDialogOpen} />
      </Box>
...
function CalendarBoard({
  ...
  emptyDayLabel: string;
  events: CenterUpCalendarEvent[];
  eventsByDate: Map<string, CenterUpCalendarEvent[]>;
  getEventTone: (event: CenterUpCalendarEvent) => CenterUpCalendarResolvedTone;
  mode: CalendarMode;
  monthMatrix: Dayjs[][];
  onEventClick: (event: CenterUpCalendarEvent, anchorEl: HTMLElement) => void;
  selectedDate: Dayjs;
  setSelectedDate: (value: Dayjs) => void;
  todayDate: string;
  visibleMonth: Dayjs;
...
function EventChip({
  ...
  compact?: boolean;
  event: CenterUpCalendarEvent;
  onEventClick: (event: CenterUpCalendarEvent, anchorEl: HTMLElement) => void;
  tone?: CenterUpCalendarResolvedTone;
}) {
...
function EventDetailPopover({
  ...
  event: CenterUpCalendarEvent | null;
  onClose: () => void;
}) {
        ...
        <Box sx={{ minWidth: 390 }}>
          ...
          <Box sx={{ alignItems: "center", display: "flex", gap: 1, justifyContent: "space-between", p: 2 }}>
            <Box sx={{ minWidth: 0 }}>
              ...
              <Typography sx={{ color: "#6B778C", fontSize: 12.25, mt: 0.25 }}>Có thể mở buổi dạy hoặc chỉnh lịch từ đây.</Typography>
            </Box>
            <Stack direction="row" spacing={1}>
              <Button sx={secondaryActionSx} variant="outlined">Chỉnh lịch</Button>
              <Button sx={primaryActionSx} variant="contained">Vào buổi</Button>
            </Stack>
          </Box>
        </Box>
      ) : null}
...
export function TeachingScheduleDialog({
  ...
  catalog: CenterUpScheduleCatalog;
  onClose: () => void;
  onPreview?: (draft: CenterUpScheduleDraft) => Promise<CenterUpSchedulePreviewResult> | CenterUpSchedulePreviewResult;
  onSubmit?: (draft: CenterUpScheduleDraft) => Promise<void> | void;
  open: boolean;
}) {
  const [selectedSchoolId, setSelectedSchoolId] = useState(() => catalog.schools[0]?.id ?? "");
  const [applyFrom, setApplyFrom] = useState("2026-01-19");
  const [applyTo, setApplyTo] = useState("2026-01-25");
  const [bulkNote, setBulkNote] = useState("");
  const [submitError, setSubmitError] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isPreviewing, setIsPreviewing] = useState(false);
  const [previewDraftKey, setPreviewDraftKey] = useState("");
  const [previewResult, setPreviewResult] = useState<CenterUpSchedulePreviewResult | null>(null);
  ...
  function buildScheduleDraft(resolveMode: CenterUpScheduleDraft["resolveMode"] = "REJECT") {
    ...
    const rows = scheduleRows
      .filter((row) => row.classId && row.mainTeacherId && row.subject)
      .map((row) => {
        ...
        return {
          ...
          periodStart: Number(row.period) || 1,
          ...
          weekday: mapWeekdayValue(row.weekday),
        } satisfies CenterUpScheduleDraftRow;
      });
    ...
    } satisfies CenterUpScheduleDraft;
  }
...
function SchedulePreviewPanel({
  ...
  result: CenterUpSchedulePreviewResult;
...
function countPreviewRowErrors(result: CenterUpSchedulePreviewResult) {
  return result.rowResults.reduce((total, row) => total + row.errors.length, 0);
}

function buildScheduleDraftKey(draft: CenterUpScheduleDraft) {
...
function calculateInclusiveRepeatWeeks(applyFrom: string, applyTo: string) {
...
function buildLaneSummaries(lanes: string[], events: CenterUpCalendarEvent[]): CenterUpCalendarLaneSummary[] {
...
function groupScheduleEvents(events: CenterUpCalendarEvent[]) {
  ...
  const groups: Array<{ date: string; items: CenterUpCalendarEvent[] }> = [];
...
function formatScheduleTimeRange(event: CenterUpCalendarEvent) {
...
function formatScheduleClassLine(event: CenterUpCalendarEvent) {
...
function resolveScheduleStatusLabel(event: CenterUpCalendarEvent) {
...
function getScopeOptionValue(summary: CenterUpCalendarLaneSummary) {
...
function getEventHour(event: CenterUpCalendarEvent) {
...
function isAllDayLikeEvent(event: CenterUpCalendarEvent) {
...
function formatCompactEventLabel(event: CenterUpCalendarEvent) {
...
function optionLabel(option: string | CenterUpScheduleOption) {
  return typeof option === "string" ? option : option.name;
}

function getOptionId(option: string | CenterUpScheduleOption) {
  return typeof option === "string" ? option : option.id;
}

function findScheduleOption(options: Array<string | CenterUpScheduleOption>, value: string) {
  return options.find((option) => getOptionId(option) === value || optionLabel(option) === value);
}

function mapWeekdayValue(value: string) {
  const normalized = value.trim().toLowerCase();
  if (normalized.includes("2")) return 1;
  if (normalized.includes("3")) return 2;
  if (normalized.includes("4")) return 3;
  if (normalized.includes("5")) return 4;
  if (normalized.includes("6")) return 5;
  if (normalized.includes("7")) return 6;
  return 7;
}

function mapSessionValue(value: string) {
  const normalized = value.trim().toLowerCase();
...
function normalizePeriodStart(periodCountValue: string, periodStartValue: string) {
  const periodCount = Math.max(1, Math.min(4, Number(periodCountValue) || 1));
  const periodStart = Math.max(1, Number(periodStartValue) || 1);
  return String(Math.min(periodStart, maxPeriodNumber - periodCount + 1));
}

function addPeriodsToTime(startTime: string, periodCountValue: string) {
...
function createInitialScheduleRows(classes: CenterUpScheduleClass[], catalog: CenterUpScheduleCatalog): ScheduleDraftRow[] {
  const subjects = catalog.subjects ?? [];
  const levels = catalog.levels ?? [];
  const rooms = catalog.rooms ?? [];
...
const scheduleStatusIconSx = (tone: CenterUpCalendarResolvedTone) => ({
  alignItems: "center",
...
const scheduleCardSx = (tone: CenterUpCalendarResolvedTone) => ({
...
function eventDetailHeaderSx(tone: CenterUpCalendarTone) {
