...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "group/command-item relative flex cursor-default items-center gap-2 rounded-md px-2.5 py-2 text-sm font-semibold outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-selected:bg-[var(--erg-blue-light)] data-selected:text-[var(--erg-blue)] in-data-[slot=dialog-content]:rounded-lg data-selected:*:[svg]:text-[var(--erg-blue)] [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
...
function CommandShortcut({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn("ml-auto text-xs text-slate-500 group-data-selected/command-item:text-[var(--erg-blue)]", className)}
      {...props}
    />
  );
}
