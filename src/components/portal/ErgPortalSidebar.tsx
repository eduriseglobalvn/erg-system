...
import type { MenuGroup, MenuItem, PortalInfo } from "./ErgPortalLayout";
import {
  COLLAPSED_SIDEBAR_WIDTH,
  EXPANDED_SIDEBAR_WIDTH,
  PORTAL_SIDEBAR_MOTION_MS,
  PORTAL_SIDEBAR_TRANSITION,
} from "./portal-sidebar-motion";
...
const SIDEBAR_MOTION_MS = PORTAL_SIDEBAR_MOTION_MS;
const SIDEBAR_MOTION_CURVE = PORTAL_SIDEBAR_TRANSITION;
const SIDEBAR_FADE_EASING = "180ms cubic-bezier(0.32, 0.72, 0, 1)";
const SUBMENU_MOTION_EASING = "240ms cubic-bezier(0.24, 0.9, 0.32, 1)";
const SUBMENU_DISCLOSURE_TRANSITION =
...
export default function ErgPortalSidebar({
  collapsed,
  menuGroups,
  portalInfo,
  onCollapsedChange,
  ...
  return (
    ...
    >
      ...
      >
        <ChevronLeftRoundedIcon
          sx={{
            ...
            transition: `transform ${SIDEBAR_MOTION_CURVE}`,
      ...
      >
        <SidebarLogo collapsed={false} active={!collapsed} onNavigate={onNavigate} />
        <SidebarLogo collapsed active={collapsed} onNavigate={onNavigate} />
      </Box>
      ...
      >
        <SidebarNavigationLayer
          active={!collapsed}
          collapsed={false}
          currentPath={currentPath}
          hideSidebarBottom={hideSidebarBottom}
  ...
  return (
    <Box
      ...
      sx={{
        ...
        transform: active
          ? "translate3d(0, 0, 0)"
          : `translate3d(${collapsed ? -6 : 6}px, 0, 0)`,
        transition: `opacity ${SIDEBAR_FADE_EASING}, transform ${SIDEBAR_MOTION_CURVE}`,
    ...
    >
      <Box
        ...
        sx={{
          ...
          m: 0,
          p: 0,
          width: collapsed ? 38 : 92,
        }}
      >
        <Box
          component="img"
          alt="ERG"
          src={collapsed ? ERG_ASSETS.mobileLogo : ERG_ASSETS.logo}
          sx={{
            height: 34,
            objectFit: "contain",
            objectPosition: collapsed ? "center" : "left center",
            width: collapsed ? 32 : 86,
          }}
        />
      </Box>
    </Box>
...
function SidebarNavigationLayer({
  active,
  collapsed,
  currentPath,
  hideSidebarBottom,
    ...
    >
      <Box
        ...
        sx={{
          ...
          flexDirection: "column",
          pb: 2,
          px: collapsed ? 0.45 : 1.15,
          width: "100%",
          "& > .erg-nav-group + .erg-nav-group": {
            mt: collapsed ? 1 : 0.5,
          },
        }}
      >
        {menuGroups.map((group, groupIndex) => (
          <Box key={groupIndex} className="erg-nav-group" sx={{ width: "100%" }}>
            {group.label && !collapsed ? (
              <Box sx={{ height: 42, overflow: "hidden" }}>
                <Typography
                  component="div"
                  sx={{
                    alignItems: "center",
...
function SidebarItem({
  ...
  if (hasChildren) {
    ...
    const defaultExpanded = childActive || isPinnedOpen;
    const isExpanded = manualOpen ?? defaultExpanded;
    const isBranchActive = isActive;
    const isPreviewOpen = isExpanded && !childActive;
    ...
    return (
      ...
      >
        <ListItemButton
          ...
          disableRipple
          onClick={() => setManualOpen((value) => !(value ?? defaultExpanded))}
          sx={{
            alignItems: "center",
        ...
        >
          ...
          <KeyboardArrowRightRoundedIcon
            sx={{
              ...
              transition: `transform ${SUBMENU_DISCLOSURE_TRANSITION}`,
              "@media (prefers-reduced-motion: reduce)": {
        ...
        <Box
          ...
          sx={{
            display: "grid",
            gridTemplateRows: showExpandedChildren ? "1fr" : "0fr",
            overflow: "hidden",
            pl: 1.5,
            transition: `grid-template-rows ${SUBMENU_DISCLOSURE_TRANSITION}`,
            "@media (prefers-reduced-motion: reduce)": {
              transitionDuration: "1ms",
            },
          }}
